<template>
    ...
    >
      ...
      <v-card-actions height="32px">
        <div v-if="event.name" class="text-decoration-none caption width">
      ...
      <v-dialog
        ...
        <div v-if="event" >
        <v-img class="my-4 mx-auto" v-if="event.poster" max-height="400px" contain :src="event.poster">
        ...
        <h3 class="font-weight-medium  d-inline">{{event.name}}</h3><span class="d-inline float-right "> <country-flag size='normal'  :country= 'event.country' /> </span>
        </div>
        </v-container>
    </v-dialog> 
    </v-card>
