<template>
    <v-hover v-slot:default="{ hover }">
      <v-card 
        data-view v-show="workshop && workshop.workshop"
        :elevation="hover ? 6 : 0"
      ...
      >
        <v-img
          ...
          <v-btn v-if="isAuthenticated && workshop.guest == loggedInUser.username" icon small class="float-right ma-1 white" 
        ...
        <v-dialog
          ...
          <v-img class="my-4 mx-auto" v-if="workshop.workshop.poster" max-height="400px" contain :src="workshop.workshop.poster">
            ...
            <v-btn v-if="isAuthenticated && workshop.guest == loggedInUser.username" icon small class="float-right ma-2 ml-0 white" 
            @click.stop="addToJourney">
            <v-icon color="black" small>mdi-plus</v-icon>
            </v-btn>
          </v-img>
